  ...
  private func setupCamera() {
    ...
    do {
      ...
      videoOutput.setSampleBufferDelegate(
        self, queue: DispatchQueue(label: "camera.frame.processing.queue"))
...
extension ExpoVisionImageScannerController: AVCaptureVideoDataOutputSampleBufferDelegate {
  ...
  private func calculateSimilarity(rect1: VNRectangleObservation, rect2: VNRectangleObservation)
    -> Double
  {
...
extension ExpoVisionImageScannerController: AVCapturePhotoCaptureDelegate {
  ...
  private func autoCropImage(_ image: UIImage) -> UIImage {
    ...
    let bottomRight = CGPoint(
      x: rectangle.bottomRight.x * width, y: rectangle.bottomRight.y * height)
